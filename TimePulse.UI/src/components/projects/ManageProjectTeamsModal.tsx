...
import { X, Users2, Search, Loader2, Check, Save } from 'lucide-react'
import { projectApi, type ProjectDto } from '../../api/projectApi'
import type { TeamDto } from '../../api/teamApi'

interface ManageProjectTeamsModalProps {
  isOpen: boolean
...
export function ManageProjectTeamsModal({
  isOpen,
  project,
  allTeams,
  onClose,
  onTeamsUpdated,
}: ManageProjectTeamsModalProps) {
  ...
  useEffect(() => {
    if (project) {
      setSelectedTeamIds(project.teams?.map((t) => t.teamId) || [])
      setSearchQuery('')
      setError(null)
    }
  }, [project])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      await projectApi.setProjectTeams(project.id, selectedTeamIds)
      onTeamsUpdated()
      onClose()
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed to update project teams.')
    } finally {
      setIsLoading(false)
    }
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">
                Assign Teams: {project.name}
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Select teams authorized to work on this project ({selectedTeamIds.length} assigned).
              </p>
            </div>
          </div>
          ...
          <div className="p-6 space-y-4 flex-1 overflow-y-auto">
            ...
            <div className="space-y-1.5 max-h-72 overflow-y-auto pr-1">
              {filteredTeams.length === 0 ? (
                <p className="text-center py-6 text-xs text-slate-400">No teams found.</p>
              ) : (
                filteredTeams.map((t) => {
