...
import { X, FolderKanban, Search, Loader2, Check, Save, Hash, Building2 } from 'lucide-react'
...
interface ManageTeamProjectsModalProps {
  isOpen: boolean
  team: TeamDto | null
...
export function ManageTeamProjectsModal({
  isOpen,
  team,
  allProjects,
  onClose,
  onProjectsUpdated,
}: ManageTeamProjectsModalProps) {
  ...
  useEffect(() => {
    if (team) {
      setSelectedProjectIds(team.projects.map((p) => p.projectId))
      setSearchQuery('')
      setError(null)
    }
  }, [team])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      await teamApi.setTeamProjects(team.id, selectedProjectIds)
      onProjectsUpdated()
      onClose()
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed to update team projects.')
    } finally {
      setIsLoading(false)
    }
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">
                Assign Projects: {team.name}
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Map projects to this team ({selectedProjectIds.length} assigned).
              </p>
            </div>
          </div>
          ...
          <div className="p-6 space-y-4 flex-1 overflow-y-auto">
            ...
            <div className="space-y-1.5 max-h-72 overflow-y-auto pr-1">
              {filteredProjects.length === 0 ? (
                <p className="text-center py-6 text-xs text-slate-400">No projects found.</p>
              ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <div
                          className="w-8 h-8 rounded-lg font-bold text-xs flex items-center justify-center shrink-0 text-white"
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {p.code && (
                              <span className="inline-flex items-center gap-0.5 px-1.5 py-0.2 rounded text-[10px] font-mono bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
